...
const compareTests = async function () {
  ...
  console.log(
    `${resultsFile1}: ${results1.designPattern}, ${results1.assistiveTechnology.name} ${results1.assistiveTechnology.version}, ${results1.browser.name} ${results1.browser.version}`,
  );
  console.log(
    `${resultsFile2}: ${results2.designPattern}, ${results2.assistiveTechnology.name} ${results2.assistiveTechnology.version}, ${results2.browser.name} ${results2.browser.version}`,
  );
...
`;
          ...
          let answer = SAVE
            ? await chooseResult(formatAssertionResult(rByAssertion[0][a]), formatAssertionResult(rByAssertion[1][a]))
            : 0;
      ...
      const ub1 = rByCommand[0][c].unexpected_behaviors.length
        ? rByCommand[0][c].unexpected_behaviors.join('; ')
        : 'none';
      const ub2 = rByCommand[1][c].unexpected_behaviors.length
        ? rByCommand[1][c].unexpected_behaviors.join('; ')
        : 'none';
      if (ub1 !== ub2) {
...
`;
  ...
  if (SAVE) {
    fse.writeFileSync(
      `revised-${results1.assistiveTechnology.name}-${results1.assistiveTechnology.version}-${results1.browser.name}-${results1.browser.version}.json`,
